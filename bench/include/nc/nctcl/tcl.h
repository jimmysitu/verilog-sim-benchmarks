...
#      define TCL_VARARGS_DEF(type, name) (va_alist) va_dcl
...
#      endif
#if ( OSCHIP_REV==aix5_3_0_rs6000 && defined(_LARGE_FILES))
 ...
 */
#   ifndef TCL_LL_MODIFIER
#if defined(HAVE_STRUCT_STAT64) || ( OSCHIP_REV==aix5_3_0_rs6000 && defined(_LARGE_FILES))
typedef struct stat64	Tcl_StatBuf;
#      else
typedef struct stat	Tcl_StatBuf;
...
#define TCL_MAYBE_EVAL_DIRECT   0x100000  /* PCR 803201 */
...
typedef struct Tcl_Token {
    ...
    int no_opnd;                /* will be one in case of RED_* opeartor which
                                 * has same symbol as some other binary
                                 * operators*/
} Tcl_Token;
...
#define TCL_TOKEN_GEN_COMMAND   256
...
#define TCL_PARSE_SYNTAX_VALSUB         10
 ...
 */
/* Commented out as it contains Windows/Mac Specific APIs only 
 * We ship this tcl.h with INCA so that users can link with this
 * and we would have to include "tclPlatDecls.h" , even though
 * we dont support either Windows or Mac
 */
