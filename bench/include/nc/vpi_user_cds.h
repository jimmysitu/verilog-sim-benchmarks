...
#define vpiGenScopeArray_OLD 518   /* array of generates */
#define vpiGenScope_OLD    519   /* generate scope */
#define vpiGenVar_OLD      520   /* generate variable */
/* type parameter moved to sv_vpi_user.h */

/* Temporary alias for deprecated item in std (1800-2009). */
#define vpiIdentifier      vpiSeqFormalDecl
#define vpiInterfaceTypespec  522 /* Typespec for interface */
#define vpiVirtualInterfaceVar   523 /* variable for VIFC */
...
#define vpiUnsupported      -2   /* known but unsupported property */
#define vpiDecompile_OLD   500   /* decompile the object */
#define vpiSigned_OLD      510   /* Object is typed as being signed */
#define vpiIsExplicitSized 523   /* Constant's size is defined explicitly */
#define vpiIsProtected_OLD  524  /* MOVED to vpi_user.h (now part of std). */
#define vpiIsModPort       527   /* whether an object is a modport */
...
#define vpiImpNamedConn    522  /* SystemVerilog implicitly named connection */
...
/*************************** range properties *****************************/
#define vpiLeftRangeVal    525
#define vpiRightRangeVal   526
...
#define vpiTimeConst           8   /* Type Time for Attributes */

/************************* expression properties **************************/
/* These can be returned by vpiOpType */
#define vpiArithLShiftOp_OLD   500
#define vpiArithRShiftOp_OLD   501
#define vpiIsImplicit          521

/* These are missing from sv_vpi_user.h - temporarily adding them here:  */
/* Moved: #define vpiInsideOp            95  */
#define vpiConstrForeach       524
...
#define vpiWholeValue          0x2000
#define vpiSingleWord          0x4000
#define vpiNoPropagate         0x8000
...
/************************ DPI access properties ****************************/
#define vpiDefaultAcc          0  /* For vpiAccessType (not in std). */
#define vpiDPIExternAcc    vpiDPIExportAcc  /* Aliased for now (remove soon!) */
...
#define vpiFileDesc        500  /* convert Verilog file descriptor into a system file pointer */
...
/* MOVED to vpi_user.h (IEEE std header):
 * cbAtEndOfSimTime   509  * to be treated like cbReadWrite,happens when no other events that modify values exist *
 * cbNBASynch         510  * occurs before any non-blocking assignments, and after all other assignments */
#define cbValChangeRWSynch 511   /* Value change callback triggering at RW synch */
#define cbValChangeROSynch 512   /* Value change callback triggering at RO synch */
#define cbDataDump         513   /* Value change callback for collections */
#define cbDDROSynch        514   /* Value change callback triggering at RO synch for collections */
#define cbGarbageCollect   515   /* Garbage collections callback triggers when class object gets garbage collected */
...
/****************** get_value and put_value by offset *********************/
#define vpiMemBuf           0x8000
#define vpiRawDataVal       14

typedef struct t_vpi_arrayval
{
  PLI_INT32 format; /* vpiRawDataVal supported as of now*/
  union
    {
      PLI_BYTE8 *memval;
    } value;
} s_vpi_arrayval, *p_vpi_arrayval;
...
XXTERN void vpi_get_value_by_offset  PROTO_PARAMS((vpiHandle arg,p_vpi_arrayval  val, int *offset, int words));
XXTERN vpiHandle vpi_put_value_by_offset  PROTO_PARAMS((vpiHandle arg, p_vpi_arrayval value_p, p_vpi_time time_p, int flags, int *offset, int words));
XXTERN void *    vpi_register_advance PROTO_PARAMS((vpiHandle obj, void (*func)(void), p_vpi_time delay));
XXTERN PLI_INT32 vpi_advance_time PROTO_PARAMS((vpiHandle obj, p_vpi_time delay));
